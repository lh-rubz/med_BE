...
class ChatResource(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            # Check if streaming is requested (default to streaming for /chat endpoint if Accept header suggests it or param)
            return Response(
                stream_with_context(self._process_multiple_images(
                    all_images_to_process,
                    current_user_id,
                    user,
                    saved_files
    ...
    def _process_multiple_images(self, images_list, current_user_id, user, saved_files):
        """Process each image separately, then combine all extracted data. Yields status updates."""
        try:
            patient_name = user.first_name + " " + user.last_name
            ...
            yield json.dumps({
                "type": "progress",
                "message": f"Starting processing of {total_pages} page(s)...",
                "percent": 0
            }) + "\n"
            ...
            for idx, image_info in enumerate(images_list, 1):
                ...
                yield json.dumps({
                    "type": "progress",
                    "message": f"Analyzing page {idx} of {total_pages}...",
                    "percent": progress_percentage,
                    "details": f"Processing file: {image_info['source_filename']}"
                }) + "\n"
...
Use the OCR text to get accurate Arabic names and values, but rely on the image to understand which values belong to which tests."""
            ...
            yield json.dumps({
                "type": "progress",
                "message": "Validating and correcting data...",
                "percent": 95
            }) + "\n"
